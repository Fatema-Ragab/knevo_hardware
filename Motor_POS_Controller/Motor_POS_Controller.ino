...
// ============================================================
// KNEVO SMOOTH FSM + CLINICAL TRAJECTORY + MOTOR OUTPUT
// ------------------------------------------------------------
// This version fixes the "tak tak" / stop-go motion problem.
// Main changes:
...
// 4) Motor command streaming is faster and non-blocking.
// 5) Actual-angle feedback is read less often so it does not interrupt motion.
// 6) ROM scaling keeps the same gait shape inside patient ROM.
// 7) ML/gait_percent bad reading logic remains: predict <=3, then freeze.
// ============================================================
...
// ============================================================
const unsigned long CONTROL_MS  = 20;   // stream motor target every 20 ms = 50 Hz
const unsigned long PLOT_MS     = 50;   // serial plotter update
const unsigned long FEEDBACK_MS = 120;  // actual angle read; slower to avoid blocking motion

const float LOOKAHEAD_PERCENT = 2.0f;   // motor follows target slightly ahead in gait cycle
const float SEND_THRESHOLD_DEG = 0.05f; // send new target if change is bigger than this

// Simulated gait input, same idea as old code but smoother with time.
// 10% per second means one full gait cycle takes about 10 seconds.
const float SIM_GAIT_SPEED_PERCENT_PER_SEC = 10.0f;
...
void sendMotorSmoothly() {
  ...
  if (fabs(commandedAngle - lastSentAngle) >= SEND_THRESHOLD_DEG) {
...
void setup() {
  ...
  Serial.println("=== KNEVO SMOOTH FSM + MOTOR ===");
  Serial.println("Smooth mode: cubic interpolation + look-ahead target + fast streaming");
...
void loop() {
  ...
  // ----------------------------------------------------------
  // 1) CONTROL UPDATE: fast and smooth
  // ----------------------------------------------------------
